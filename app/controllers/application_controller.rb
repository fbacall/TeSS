class ApplicationController < ActionController::Base
  ...
  def check_server
    ...
    unless test_ip.nil?
      ips = Socket.ip_address_list.collect(&:ip_address)
      # hostname = `hostname -i`.chomp! # I have naughtily assumed this is a linux box
      # if hostname == test_ip
      @test_server = true if ips.include? test_ip
    end
  end
  ...
  def look_for_topics(suggestible)
    if suggestible.scientific_topic_names.empty? && suggestible.edit_suggestion.nil?
