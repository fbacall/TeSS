module EventsHelper
  ...
  def google_calendar_export_url(event)
    dates = if event.all_day?
              "#{event.start.strftime('%Y%m%d')}/#{event.end.tomorrow.strftime('%Y%m%d')}" # Need to add 1 day for all day events apparently
            else
              "#{event.start.strftime('%Y%m%dT%H%M00Z')}/#{event.end.strftime('%Y%m%dT%H%M00Z')}"
            end

    event_params = {
      text: event.title,
      dates: dates,
      details: event_url(event).to_s,
  ...
  def csv_column_names
    %w(Title Organizer Start End ContentProvider)
  end
  ...
  def csv_from_collection(events)
    CSV.generate do |csv|
      ...
      events.each do |event|
        unless event.start.nil? || event.end.nil? || event.title.nil?
