  ...
  def filter_link(name, value, count, title = nil, html_options = {})
    ...
    link_to parameters, html_options do
      if block_given?
        ...
        title + content_tag(:span, count.to_s, class: 'facet-count')
      end
    end
  end

  def remove_filter_link(name, value, html_options = {}, title = nil)
    parameters = params.dup
    title ||= (title || truncate(value.to_s, length: 30))
